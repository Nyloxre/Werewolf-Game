...
function resolveVoting(room) {
  if (room.phase !== 'voting') return; // guard against double-resolution
  room.phase = 'resolving'; // prevent any further votes from re-triggering this
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'create_room') {
      ...
      if (playerRoom.dayVotes[playerName]) return; // already voted
      playerRoom.dayVotes[playerName] = msg.target;

      const living = playerRoom.clients.map(c => c.name).filter(p => playerRoom.alive[p]);
      const skipCount = Object.values(playerRoom.dayVotes).filter(v => v === 'skip').length;
      ...
      broadcastAll(playerRoom, {
        ...
        total: living.length,
        skipCount,
        majority: Math.floor(living.length / 2) + 1
      });

      // Resolve if skip has majority OR all living players have voted
      const skipMajority = skipCount > living.length / 2;
      const allVoted = living.every(p => playerRoom.dayVotes[p]);
      if (skipMajority || allVoted) {
      ...
      if (!playerRoom || playerName !== playerRoom.hostName) return;
      if (playerRoom.phase !== 'voting') return;
      resolveVoting(playerRoom);
    }
